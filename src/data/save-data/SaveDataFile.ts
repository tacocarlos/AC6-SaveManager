...
export class SaveDataFile implements SaveData {
    ...
    static async createValidSaveFile(
        meta: SaveDataFileMetadata,
        savePath: string
    ): Promise<SaveDataFile> {
    ...
    async getMetadataPath() {
        return await join(
            await dirname(this.getFilePath()),
            ARCHIVE_METADATA_NAME
        );
    }
    ...
    async setSaveActive(config: AppConfig): Promise<void> {
        if (!this.validateSave()) {
            Logger.error(
                'Save validation failed during install: ' + this.getFilePath()
            );
            return Promise.reject();
        }
        ...
        if (await exists(destPath)) {
            ...
            try {
                ...
                message(
                    'Failed to backup currently active save.\nWill not attempt to swap save files.'
                );
            }
        }
    ...
    async setMetadataPath() {
        ...
        const mdPath = await join(
            dir,
            SaveDataFileMetadata.SaveMetadataFileName
        );
    ...
    async writeSave(): Promise<void> {
        ...
        try {
            await writeTextFile(
                mdPath as string,
                JSON.stringify(this.getMetadata(), null, '\t')
            );
            return Promise.resolve();
        } catch (reason) {
            return Promise.reject();
    ...
    async updateSave(
        ...
        if (
            (await basename(sourceDataFile)) !== SAVE_NAME ||
            (await !isFile(sourceDataFile))
        ) {
            return Promise.reject();
        }
        ...
        await copy(sourceDataFile, this.getFilePath())
            .then(() => {
            ...
            .catch(() => {
                ...
                return dialog.message(
                    `Save Data Failure. Could not restore the save data from temp.\n${TEMP_SAVE_LOC}`
                );
            });
    }
