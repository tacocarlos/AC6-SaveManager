...
export function SRButton({ active, action, children, className} : 
    {active?: boolean, action: React.MouseEventHandler, children: React.ReactElement, className?: string}) {
    ...
    return <button onClick={action} className={`${styleName}`} disabled={!active}>
        {children}
    </button>
}