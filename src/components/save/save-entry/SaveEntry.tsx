...
export function SaveEntry({save, isSelected, modalToggle} : {save: SaveData, isSelected: boolean, modalToggle: any}): JSX.Element {
    ...
    const notesPreview = save.getMetadata().getNotes().substring(0 , NOTES_PREVIEW_LENGTH);    
    const selectedStyle = isSelected ? `${saveStyles.selectedSave}` : "";

    return <div onClick={selectSave} className={`${saveStyles.saveEntryWrapper} ${selectedStyle}`}>
            <h2 className={saveStyles.saveName}>
            ...
            <div className={saveStyles.noteWrapper}>
                <Markdown className={saveStyles.notePreview} remarkPlugins={[remarkGfm, remarkMath]} rehypePlugins={[]}>
                    {notesPreview + (save.getMetadata().getNotes().length > NOTES_PREVIEW_LENGTH ? "..." : "")}
                </Markdown>
            </div>
            <br/>
