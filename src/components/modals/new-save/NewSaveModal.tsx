...
export default function NewSaveModal({
    ...
    const [metadata, setMetadata] = useState(
        SaveDataFileMetadata.defaultVanillaMetadata(saveID)
    );
    ...
    async function createNewSave(sourcePath: string) {
        ...
        const saveFolder = await join(
            await folderArchive.getArchivePath(),
    ...
    function onNameChange(change: React.ChangeEvent<HTMLInputElement>) {
        ...
        if (nameRef.current) {
            ...
            setName(newName);
            setMetadata((curr) => {
                const next = SaveDataFileMetadata.defaultVanillaMetadata(
                    curr.getID()
                );
                next.updateLMDate();
                next.setNotes(note);
    ...
    const title =
        newSaveType === 'backup'
            ? 'Backup Currently Installed Save File'
        ...
        <Modal
            ...
            <form
                ...
                <label className={modalStyles.notesLabel}>
                    ...
                    <br />
                    <textarea
                        className={modalStyles.notesArea}
                        ref={noteRef}
                ...
                <br />
                <Button onClick={() => createNewSave(sourceFile)} type="submit">
                    Create New Save
                </Button>
            </form>
        </Modal>
