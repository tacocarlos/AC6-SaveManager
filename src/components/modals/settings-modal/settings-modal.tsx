...
export default function SettingsModal(props: {
    ...
    async function saveSettings() {
        try {
            ...
            Logger.trace(`Saved settings to ${CONFIG_FILE_PATH}.\n${config.stringify()}`)
        } catch(reason: any) {
            Logger.error(reason);
    ...
    async function removeArchivePath(ap: string) {
        const confirmResult = await dialog.confirm("Are you sure you want to remove\n" + ap + "\nfrom the list of paths containing archives?");
        if(!confirmResult) {
            return;
        }
    ...
    async function changeSaveLocation() {
        const defaultPath = (await isDir(AC6ROAMING) ? AC6ROAMING : DOCUMENTS_DIR);
        const selected = await dialog.open({
        ...
        try {
            ...
            await dialog.message("Invalid save location selection", {type: 'warning'});
        }
    ...
    const archivePathsContent = archivePaths.map( ap => {
        ...
        return (
            <li key={ap} className={`${modalStyles.archivePathItem} ${modalStyles.itemWrapper}`}>
    ...
    return (<Modal isOpen={props.isOpen} onRequestClose={props.modalClose} className={modalStyles.modalStyle} ariaHideApp={false}>
        <h1 className={`${modalStyles.titleHeader}`}>Armored Core 6 Save Manager Settings</h1>
        <br/>
        <div className={`${modalStyles.sectionWrapper} ${modalStyles.execSection}`}>
            <h2 className={`${modalStyles.archivePathHeader}`}>Executable Path</h2>
            <div>
                <span className={`${modalStyles.itemWrapper} ${modalStyles.execPath}`}>{exePath}</span>
                <button onClick={changeExecPath} style={{'marginRight': '10px'}}>
                    Change Executable Path
                </button>

                <button onClick={() => {
                    config.clear_path();
                }}>Clear Executable Path</button>
            </div>
        </div>
        
        <div className={`${modalStyles.archivePathListSection} ${modalStyles.sectionWrapper}`}>
            <h2 className={`${modalStyles.archivePathHeader}`}>Archive Paths</h2>
            <ul className={`${modalStyles.archivePathList}`}>
                {archivePathsContent}
            </ul>
            <button onClick={addArchivePath}> Add Archive Search Directory</button>
        </div>
        ...
        <div className={modalStyles.sectionWrapper}>
            ...
            <br/>
            <button onClick={changeSaveLocation} style={{marginTop: '10px'}}>Change Save File Location</button>
        </div>
        
        <br/>
        <div>
            <button onClick={props.closeButtonHandler} style={{'marginRight': '10px'}}>Close</button>
